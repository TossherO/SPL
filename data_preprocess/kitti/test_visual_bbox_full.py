...
if __name__ == '__main__':
    info_path = './data/kitti/_kitti_data_info.pkl'
    with open(info_path, 'rb') as f:
        data_info = pickle.load(f)

    for sample_idx in range(0, 2000, 50):
        sample = data_info['kitti_object'][sample_idx]
        raw_data = data_info['kitti_raw'][sample['scene']]
        lidar_path = os.path.join('./data/kitti/points', sample['name'] + '.bin')
        points = np.fromfile(lidar_path, dtype=np.float32).reshape(-1, 4)[:, :3]

        pseudo_labels_path = f'./data/kitti/_pseudo_labels/{sample["scene"]}.pkl'
