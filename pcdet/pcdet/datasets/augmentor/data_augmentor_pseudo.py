...
class DataAugmentor_pseudo(object):
    ...
    def random_world_flip(self, data_dict=None, config=None):
        ...
        for cur_axis in config['ALONG_AXIS_LIST']:
            ...
            if 'pseudo_boxes' in data_dict.keys():
                pseudo_boxes, _, _ = getattr(augmentor_utils, 'random_flip_along_%s' % cur_axis)(
                data_dict['pseudo_boxes'], np.zeros([1,3]), return_flip=True, enable=enable
                )
    ...
    def random_world_rotation(self, data_dict=None, config=None):
        ...
        if 'roi_boxes' in data_dict.keys():
            ...
            data_dict['roi_boxes'] = roi_boxes.reshape(num_frame, num_rois,dim)
        if 'pseudo_boxes' in data_dict.keys():
            pseudo_boxes, _, _ = augmentor_utils.global_rotation(
            data_dict['pseudo_boxes'], np.zeros([1, 3]), rot_range=rot_range, return_rot=True, noise_rotation=noise_rot)
            data_dict['pseudo_boxes'] = pseudo_boxes
    ...
    def forward(self, data_dict):
        ...
        if 'pseudo_boxes' in data_dict.keys():
            data_dict['pseudo_boxes'][:, 6] = common_utils.limit_period(
                data_dict['pseudo_boxes'][:, 6], offset=0.5, period=2 * np.pi
            )
